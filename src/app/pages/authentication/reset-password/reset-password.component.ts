...
export class ResetPasswordComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    [this.email, this.token] = String(atob(this.signature)).split(':');
  ...
  validate_link() {
    ...
    this.loginService.validate_link(this.email, this.token).subscribe({
      next: (reponse) => {
        reponse.subscribe({
          next: (response) => {
  ...
  onSubmit(form: HTMLFormElement) {
    ...
    this.loginService.reset_password(form_data, this.email).subscribe({
      next: (response) => {
